...
// NOTE Conditionally return a list of plugins to use based on the current environment. Repeat this pattern for any other config key (ie: loaders, etc).
function getPlugins() {
  var plugins = [];
  plugins.push(
    new StatsWriterPlugin({ // https://www.npmjs.com/package/webpack-stats-plugin
      filename: 'stats.json'
    }) // NOTE Causes the asset's `size` method to be called
  )

  // NOTE Conditionally add plugins for Production
  if (isProd) {
    plugins.push(
  ...
  // NOTE Conditionally add plugins for Development
  else {
  ...
  plugins: [
      new webpack.DefinePlugin({
        "process.env": {
          ...
          VERSION: JSON.stringify(gitRevision || 'NOT_GIT_REPO'),
